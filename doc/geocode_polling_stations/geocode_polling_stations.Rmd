...
We then assign longitude and latitude based on the closest match and the specificity of the match.
...
3.  Find the centroid for all unique streets and neighborhoods in the CNEFE datasets.
...
[^3]: The normalization function converts all strings to ASCII characters, transforms to lower case, and removes extraneous white space.
    ...
    We standardize common street abbreviations such as replacing "Av" with "Aveninda".
...
We pick the coordinates associated with the smallest string distance.[^4]
For matches based purely on street name or neighborhood name, we calculate the centroid of all properties on a given street or in a given neighborhood.

[^4]: For matches based on school name or address with a distance of .1 or less, we arbitrarily set the string distance to 0. This ensures that these more accurate matches are selected when only small discrepancies exist between the polling station name or address and the administrative data. In the case of ties, we select the coordinates in the following order:

    1.  Match based on school name in INEP data.
    2.  Match based on address in the INEP data.
    3.  Match based on the school name in the 2010 CNEFE data.
    4.  Match based on the school address in the 2010 CNEFE data.
    5.  Match based on the street name in the 2017 CNEFE data.
    6.  Match based on the street name in the 2010 CNEFE data.
    7.  Match based on the neighborhood name in the 2017 CNEFE data.
    8.  Match based on the neighborhood name in the 2010 CNEFE data.
...
loadd(inep_string_match, cnefe_stbairro_match, schools_cnefe_match,
      agrocnefe_stbairro_match, locais)
```
...
#example_id <- sample(locais$local_id[is.na(locais$latitude_local) == FALSE], 1)
example_id <- 94047
example_polling_station <- filter(locais, local_id == example_id)
...
table_data <- tibble(Data = c("INEP School Name", "INEP School Address",
  ...
  rowwise() %>%
  mutate(
    `Error (km)` = geosphere::distHaversine(p1 = c(example_polling_station$longitude_local, example_polling_station$latitude_local),
                                    p2 = c(long, lat))/1000 
  ) %>%
  select(!long & !lat)
...
table_data %>%
  ...
  fmt_number(matches("String Distance|Error"), 
   ...
   tab_header(
    ...
    subtitle = paste0("Polling Station Name is ", example_polling_station$local_votacao, 
                      ". Polling Station Address is ", example_polling_station$endereco)
  ) %>%
  tab_style(
...
geocoded_locais <-  left_join(locais, best_string_match) %>%
  filter(!is.na(latitude_local))

rural_urban <-  st_as_sf(geocoded_locais, coords = c("longitude_local", "latitude_local"), crs = 4674) %>%
  st_join(tract_shp, left = TRUE, largest = TRUE) %>%
  st_drop_geometry()  %>%
  as_tibble() %>%
  select(local_id, zone)

geocoded_locais <- left_join(geocoded_locais, rural_urban)

google_geocoded_df <- select(google_geocoded_df, local_id, lon, lat) %>%
  rename("google_long" = "lon", "google_lat" = "lat")

geocoded_locais <- left_join(geocoded_locais, google_geocoded_df)
```

To have some sense of the error of our procedure, we use a subset of `r nrow(geocoded_locais)` with coordinates to check the error of the coordinates generated by our procedure.
We obtained these coordinates from the *Estado de Sāo Paulo*, which used this data for a story entitled "[Como votou sua vizinhança?](https://www.estadao.com.br/infograficos/politica,como-votou-sua-vizinhanca-explore-o-mapa-mais-detalhado-das-eleicoes,935858)". 
...
error <-  geocoded_locais %>%
  rowwise() %>%
  mutate(dist = geosphere::distHaversine(p1 = c(longitude_local, latitude_local),
                                         p2 = c(long, lat))/1000,
         google_dist = geosphere::distHaversine(p1 = c(longitude_local, latitude_local),
...
The error rate by matching variable is plotted above. As expected, error rate tends to be better when we are able to match to a specific school or address. 
...
That said, the relationship is noisy and string distance serves only as a rough guide for error. The visible discontinuity at a string distance of .1 is an artifact of the fact that we allow for more error when matching polling stations by school name or address, since matches with small discrepancies still tend to be genuine matches. 
