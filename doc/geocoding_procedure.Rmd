...
example_polling_station <- as.data.frame(geocoded_locais[local_id == example_id])
example_inep <- as.data.frame(inep_string_match[local_id == example_id])
example_cnefe10_schools <- as.data.frame(schools_cnefe10_match[local_id == example_id])
example_cnefe22_schools <- as.data.frame(schools_cnefe22_match[local_id == example_id])
example_agrocnefe <- as.data.frame(agrocnefe_stbairro_match[local_id == example_id])
example_cnefe10 <- as.data.frame(cnefe10_stbairro_match[local_id == example_id])
example_cnefe22 <- as.data.frame(cnefe22_stbairro_match[local_id == example_id])
example_string_match <- tibble(match_type = c(
  ...
  left_join(as.data.frame(model_predictions[local_id == example_id]))
...
table_data <- tibble(
  ...
  Match = c(
    example_inep$match_inep_name,
    example_inep$match_inep_addr,
    example_cnefe10_schools$match_schools_cnefe_name,
    example_cnefe10_schools$match_schools_cnefe_addr,
    example_cnefe22_schools$match_schools_cnefe_name,
    example_cnefe22_schools$match_schools_cnefe_addr,
    example_agrocnefe$match_st_cnefe,
    example_cnefe10$match_st_cnefe,
    example_cnefe22$match_st_cnefe,
    example_agrocnefe$match_bairro_cnefe,
    example_cnefe10$match_bairro_cnefe,
    example_cnefe22$match_bairro_cnefe
  ),
  `String Distance` = c(
    example_inep$mindist_name_inep,
    example_inep$mindist_addr_inep,
    example_cnefe10_schools$mindist_name_schools_cnefe,
    example_cnefe10_schools$mindist_addr_schools_cnefe,
    example_cnefe22_schools$mindist_name_schools_cnefe,
    example_cnefe22_schools$mindist_addr_schools_cnefe,
    example_agrocnefe$mindist_st_cnefe,
    example_cnefe10$mindist_st_cnefe,
    example_cnefe22$mindist_st_cnefe,
    example_agrocnefe$mindist_bairro_cnefe,
    example_cnefe10$mindist_bairro_cnefe,
    example_cnefe22$mindist_bairro_cnefe
  ),
  `Predicted Error (km)` = example_string_match$pred_dist,
  long = c(
    example_inep$match_long_inep_name,
    example_inep$match_long_inep_addr,
    example_cnefe10_schools$match_long_schools_cnefe_name,
    example_cnefe10_schools$match_long_schools_cnefe_addr,
    example_cnefe22_schools$match_long_schools_cnefe_name,
    example_cnefe22_schools$match_long_schools_cnefe_addr,
    example_agrocnefe$match_long_st_cnefe,
    example_cnefe10$match_long_st_cnefe,
    example_cnefe22$match_long_st_cnefe,
    example_agrocnefe$match_long_bairro_cnefe,
    example_cnefe10$match_long_bairro_cnefe,
    example_cnefe22$match_long_bairro_cnefe
  ),
  lat = c(
    example_inep$match_lat_inep_name,
    example_inep$match_lat_inep_addr,
    example_cnefe10_schools$match_lat_schools_cnefe_name,
    example_cnefe10_schools$match_lat_schools_cnefe_addr,
    example_cnefe22_schools$match_lat_schools_cnefe_name,
    example_cnefe22_schools$match_lat_schools_cnefe_addr,
    example_agrocnefe$match_lat_st_cnefe,
    example_cnefe10$match_lat_st_cnefe,
    example_cnefe22$match_lat_st_cnefe,
    example_agrocnefe$match_lat_bairro_cnefe,
    example_cnefe10$match_lat_bairro_cnefe,
    example_cnefe22$match_lat_bairro_cnefe
  )
) |>
...
google_geocoded_df <- readr::read_csv("./data/google_geocoded.csv") |>
...
rural_urban <- st_as_sf(filter(google_geocoded_df, !is.na(long)),
...
To estimate the accuracy of our procedure, we use a subset of `r format(nrow(google_geocoded_df), nsmall=1, big.mark=",")` polling stations with coordinates to check the error of the coordinates generated by our procedure.  These "ground truth" coordinates were provided by the TSE, which for 2018 has coordinates for `r format(sum(geocoded_locais$ano == 2018 & !is.na(geocoded_locais$tse_long)), nsmall=1, big.mark = ",")`. We used the Google Maps API (as of October 2020) to geocode a large subset of these polling stations to compare our error rate to theirs.  We report the error rate at 3 quantiles: 25th, 50th (the median), and the 75th percentile.
...
The difference is particularly large for rural polling stations, where our median error rate is `r round(median(error$dist[error$zone == "RURAL"], na.rm = TRUE), 2)` km and the Google median error rate is `r round(median(error$google_dist[error$zone == "RURAL"], na.rm = TRUE), 2)` km, more than a 30 fold difference.
