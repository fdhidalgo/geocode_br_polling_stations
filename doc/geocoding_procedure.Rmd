...
tar_load(
  c(inep_string_match, cnefe_stbairro_match, schools_cnefe_match,
...
The difference is particularly large for rural polling stations, where our median error rate is `r round(median(error$dist[error$zone == "RURAL"], na.rm = TRUE), 2)` km and the Google median error rate is `r round(median(error$google_dist[error$zone == "RURAL"], na.rm = TRUE), 2)` km, more than a 10 fold difference.
