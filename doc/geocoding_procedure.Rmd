...
To estimate the accuracy of our procedure, we use a subset of `r format(nrow(google_geocoded_df), nsmall=1, big.mark=",")` polling stations with coordinates to check the error of the coordinates generated by our procedure.  These "ground truth" coordinates were provided by the TSE, which for 2018 has a coordinates for `r sum(geocoded_locais$ano == 2018 & !is.na(geocoded_locais$tse_long))`. We used the Google Maps API to geocode a large subset of these polling stations to compare our error rate to theirs.  We report the error rate at 3 quantiles: 25th, 50th (the median), and the 75th percentile.
