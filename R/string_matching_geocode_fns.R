match_inep_muni <- function(locais_muni, inep_muni) {
  ...
  # Match on inep name
  name_inep_dists <- stringdist::stringdistmatrix(locais_muni$normalized_name,
    inep_muni$norm_school,
    method = "lv"
  )
  # Normalize for string length
  name_inep_dists <- name_inep_dists / outer(nchar(locais_muni$normalized_name),
  ...
  data.table(
    local_id, match_inep_name, mindist_name_inep, match_long_inep_name, match_lat_inep_name,
...
match_schools_cnefe_muni <- function(locais_muni, schools_cnefe_muni) {
  ...
  name_schools_cnefe_dists <- stringdist::stringdistmatrix(locais_muni$normalized_name,
  ...
  data.table(
    local_id, match_schools_cnefe_name, mindist_name_schools_cnefe,
...
match_stbairro_cnefe_muni <- function(locais_muni, cnefe_st_muni, cnefe_bairro_muni) {
  ...
  data.table(
    local_id, mindist_st_cnefe, match_st_cnefe, match_long_st_cnefe, match_lat_st_cnefe,
...
match_stbairro_agrocnefe_muni <- function(locais_muni, agrocnefe_st_muni, agrocnefe_bairro_muni) {
  ...
  locais_muni$normalized_st <- str_replace(locais_muni$normalized_st, ",.*$| sn$", "")
  # Calculate the string distance between normalized street names
  st_agrocnefe_dists <- stringdist::stringdistmatrix(locais_muni$normalized_st,
    agrocnefe_st_muni$norm_street,
    method = "lv"
  )
  # Normalize for string length
  st_agrocnefe_dists <- st_agrocnefe_dists / outer(nchar(locais_muni$normalized_st),
  ...
  locais_muni[normalized_bairro == "", normalized_bairro := normalized_st]
  # Calculate the string distance between normalized bairro names
  bairro_agrocnefe_dists <- stringdist::stringdistmatrix(locais_muni$normalized_bairro,
    agrocnefe_bairro_muni$norm_bairro,
    method = "lv"
  )
  # Normalize for string length
  bairro_agrocnefe_dists <- bairro_agrocnefe_dists / outer(nchar(locais_muni$normalized_bairro),
  ...
  out_data <- data.table(
    local_id, mindist_st_agrocnefe, match_st_agrocnefe,
...
make_model_data <- function(cnefe10_stbairro_match, cnefe22_stbairro_match,
                            schools_cnefe10_match, schools_cnefe22_match,
                            agrocnefe_stbairro_match, inep_string_match,
                            muni_demo, muni_area, locais, tsegeocoded_locais) {
  ...
  cnefe_stbairro_match <- list(cnefe10_stbairro_match, cnefe22_stbairro_match, agrocnefe_stbairro_match) |>
    rbindlist()
  schools_cnefe_match <- list(schools_cnefe10_match, schools_cnefe22_match) |>
    rbindlist()

  # Melt the CNEFE data to long format
  cnefe_stbairro_match <- melt(cnefe_stbairro_match,
    id.vars = c("local_id", "ano"),
    measure.vars = patterns(long = "long", lat = "lat", mindist = "mindist"),
    variable.name = "type", value.name = "value",
    variable.factor = FALSE
  )
  cnefe_stbairro_match[, type := paste0(fifelse(type == 1, "st_cnefe", "bairro_cnefe"), "_", ano)]
  cnefe_stbairro_match[, ano := NULL]

  # Melt the schools CNEFE data to long format
  schools_cnefe_match <- melt(schools_cnefe_match,
    id.vars = c("local_id", "ano"),
    measure.vars = patterns(long = "long", lat = "lat", mindist = "mindist"),
    variable.name = "type", value.name = "value",
    variable.factor = FALSE
  )
  schools_cnefe_match[, type := paste0(fifelse(type == 1, "schools_cnefe_name", "schools_cnefe_addr"), "_", ano)]
  schools_cnefe_match[, ano := NULL]

  # Melt the INEP data to long format
  inep_string_match <- melt(inep_string_match,
    id.vars = c("local_id"),
    measure.vars = patterns(long = "long", lat = "lat", mindist = "mindist"),
    variable.name = "type", value.name = "value",
    variable.factor = FALSE
  )
  inep_string_match[, type := fifelse(type == 1, "schools_inep_name", "schools_inep_addr")]

  # Prepare municipal demographic data
  muni_demo[, logpop := log(POP)]
  muni_demo[, pct_rural := 100 * pesoRUR / POP]
  muni_demo <- muni_demo[ANO == 2010, .(cod_localidade_ibge = Codmun7, logpop, pct_rural)]
  ...
  addr_features <- locais[, .(locais, local_id, nm_locvot, ds_endereco, ds_bairro, normalized_addr, normalized_name)]
  addr_features[, norm_name := stringi::stri_trans_general(nm_locvot, "Latin-ASCII") |>
    str_to_lower() |>
    str_remove_all("\\.") |>
    str_remove_all("[[:punct:]]") |>
    str_squish()]
  addr_features[, centro := fifelse(grepl("\\bcentro\\b", normalized_addr) == TRUE, 1, 0)]
  addr_features[, zona_rural := fifelse(grepl("\\brural\\b", ds_endereco, ignore.case = TRUE, useBytes = TRUE) == TRUE |
    grepl("\\brural\\b", ds_bairro, ignore.case = TRUE, useBytes = TRUE) == TRUE, 1, 0)]
  addr_features[, school := fifelse(grepl(paste0("\\b", school_syns, "\\b", collapse = "|"), norm_name) == TRUE, 1, 0)]
  addr_features[, length_norm_name := nchar(norm_name)]
  addr_features[, length_norm_addr := nchar(normalized_addr)]

  addr_features <- addr_features[, .(local_id, centro, zona_rural, school, length_norm_name, length_norm_addr)]

  # Combine matching data from multiple sources
  matching_data <- rbindlist(list(cnefe_stbairro_match, schools_cnefe_match, inep_string_match)) |>
    merge(locais[, .(local_id, ano, cod_localidade_ibge)], all.x = TRUE, all.y = FALSE) |>
  ...
  model_data <- merge(tsegeocoded_locais[, .(local_id, tse_lat, tse_long)], matching_data,
    by = "local_id", all.x = TRUE, all.y = TRUE
  )
  ## Calculate the distance between the geocoded data and the matching data in kilometers
  model_data[, dist := geosphere::distHaversine(cbind(long, lat), cbind(tse_long, tse_lat),
    r = 6378.137
  )]
  model_data[, ano := NULL]
...
make_model_data <- function(cnefe10_stbairro_match, cnefe22_stbairro_match,
  ...
  model_data <- merge(tsegeocoded_locais[, .(local_id, tse_lat, tse_long)], matching_data, by = "local_id", all.x = TRUE, all.y = TRUE)
  model_data[, dist := geosphere::distHaversine(cbind(long, lat), cbind(tse_long, tse_lat), r = 6378.137)]
  model_data[, ano := NULL]
  model_data[, tse_lat := NULL]
  model_data[, tse_long := NULL]
...
train_model <- function(model_data, grid_n = 10, sample = NULL) {
  ...
  splits <- rsample::group_initial_split(model_data,
  ...
  vfolds <- rsample::group_vfold_cv(model_data,
    group = cod_localidade_ibge,
    v = 10
  )
  ...
  gbm_recipe <- recipes::recipe(
    ...
    recipes::update_role(cod_localidade_ibge,
