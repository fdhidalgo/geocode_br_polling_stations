...
tar_load(
    c(
      ...
      agrocnefe_stbairro_match, locais, model_predictions, geocoded_locais
    ))
...
table_data <- ex_pred[, .(
  ...
  select(
    ...
    `Expected Error (km)`, `Error Bound (km)`, `True Error (km)`, rank
  )

# Fail loud if the row highlighted below is not the one the pipeline actually shipped.
winner <- ex_pred[order(pred_logmean)][1]
stopifnot(
  "highlighted candidate is not the pipeline's pick" =
    isTRUE(all.equal(c(winner$long, winner$lat), c(example_pick$pred_long, example_pick$pred_lat)))
)
