...
make_model_data <- function(cnefe10_stbairro_match, cnefe22_stbairro_match,
  ...
  ## Turn CNEFE neighborhood and daddress data into predictors for model
  ...
  cnefe_stbairro_match[, type :=
    paste0(fifelse(type == 1, "st_cnefe", "bairro_cnefe"), "_", ano)]
  cnefe_stbairro_match[, ano := NULL]


  ## Turn CNEFE school data into predictors for model
  ...
  addr_features <- locais[, .(
    locais, local_id, nm_locvot,
    ds_endereco, ds_bairro, normalized_addr, normalized_name
  )]
  ...
  addr_features[, norm_name := stringi::stri_trans_general(nm_locvot, "Latin-ASCII") |>
    ...
    str_squish()]
  addr_features[, centro := fifelse(grepl(
    "\\bcentro\\b",
    normalized_addr
  ) == TRUE, 1, 0)]
  addr_features[, zona_rural := fifelse(grepl("\\brural\\b",
    ds_endereco,
    ignore.case = TRUE, useBytes = TRUE
  ) == TRUE |
    grepl("\\brural\\b", ds_bairro,
      ignore.case = TRUE,
      useBytes = TRUE
    ) == TRUE, 1, 0)]
  addr_features[, school := fifelse(grepl(paste0("\\b", school_syns,
    "\\b",
    collapse = "|"
  ), norm_name) == TRUE, 1, 0)]
  ...
  addr_features <- addr_features[, .(
    local_id, centro, zona_rural, school,
    length_norm_name, length_norm_addr
  )]

  matching_data <- rbindlist(list(
    cnefe_stbairro_match, schools_cnefe_match, inep_string_match
  )) |>
    merge(locais[, .(local_id, ano, cod_localidade_ibge)],
  ...
  model_data <- merge(tsegeocoded_locais[, .(local_id, tse_lat, tse_long)],
    matching_data,
    by = "local_id", all.x = TRUE, all.y = TRUE
  )
  model_data[, dist := geosphere::distHaversine(
    cbind(long, lat),
