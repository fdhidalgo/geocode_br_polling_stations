...
clean_cnefe22 <- function(cnefe22_file, muni_ids) {
  ...
  addr <- cnefe22[, .(
    id_munic_7, id_TSE, municipio, especie_lab, street,
    address, bairro = dsc_localidade, desc = dsc_estabelecimento,
    cnefe_long = longitude, cnefe_lat = latitude
  )]
...
clean_tsegeocoded_locais <- function(tse_files, muni_ids, locais) {
  ...
  # IMPORTANT: TSE data has one row per voting SECTION (nr_secao), but we need one row per LOCATION
  # By excluding nr_secao from selected columns and calling unique(), we consolidate multiple sections
  # into a single row per polling location. This is intentional and correct behavior.
  ...
  # Store initial count for validation reporting
  n_initial <- nrow(locs)
  message(sprintf("TSE geocoded data: Processing %d records with coordinates", n_initial))
  
  # VALIDATION POINT 1: Merge with municipality IDs
  message("Validating TSE-Municipality merge...")
  
  # Prepare municipality data for merge
  muni_for_merge <- muni_ids[, .(id_munic_7, cd_municipio = id_TSE)]
  
  # Perform validated merge
  locs <- safe_merge(
    dt1 = locs,
    dt2 = muni_for_merge,
    keys = "cd_municipio",
  ...
  na_muni <- locs[is.na(id_munic_7), .N]
  if (na_muni > 0) {
    warning(sprintf("TSE-Municipality merge: %d records (%.1f%%) did not match", 
                   na_muni, na_muni/nrow(locs)*100))
  }
  ...
  # VALIDATION POINT 2: Merge with locais (composite key - most critical!)
  message("Validating TSE-Locais composite key merge...")
  
  # Prepare data for merge
  locais_for_merge <- locais[, .(local_id, ano, cod_localidade_ibge, nr_zona, nr_locvot)]
  merge_keys <- c("ano", "cod_localidade_ibge", "nr_zona", "nr_locvot")
  
  # Note: TSE data has one row per voting section, so "duplicates" are expected
  # The unique() call on line 119 consolidates sections into locations
  # No warning needed as this is normal behavior
  
  n_before_merge <- nrow(locs)
  
  # Perform validated merge
  # Note: join_type is "many-to-one" because TSE locations may appear in multiple years
  locs <- safe_merge(
    dt1 = locs,
    dt2 = locais_for_merge,
    keys = merge_keys,
    join_type = "many-to-one",  # TSE locations can match multiple locais records across years
    merge_type = "left",        # Keep all TSE records
    validate_before = TRUE,
    validate_after = TRUE,
    stop_on_error = FALSE
  )
  ...
  # Report match rate
  n_matched <- locs[!is.na(local_id), .N]
  match_rate <- n_matched / n_before_merge
  message(sprintf("TSE-Locais match rate: %.1f%% (%d of %d records matched)", 
                 match_rate * 100, n_matched, n_before_merge))
  
  if (match_rate < 0.5) {
    warning("Low TSE-Locais match rate (<50%) - check composite keys!")
  }
  ...
  n_before_dedup <- nrow(locs)
  locs <- locs[locs[, .I[which.max(ano)], by = local_id]$V1]
  n_after_dedup <- nrow(locs)
  
  if (n_before_dedup > n_after_dedup) {
    message(sprintf("Removed %d older duplicate records (kept most recent year per local_id)", 
                   n_before_dedup - n_after_dedup))
  }
  
  # Final summary
  message(sprintf("TSE geocoding complete: %d unique locations with coordinates", nrow(locs)))
  
  locs
}

clean_agro_cnefe <- function(agro_path, muni_ids) {
  agro_cnefe <- fread(agro_path, encoding = "UTF-8")
...
normalize_school <- function(x) {
  ...
  result <- str_remove_all(result, "[[:punct:]]")
  result <- str_squish(result)
  result <- str_remove_all(result, paste0("\\b", school_syns, "\\b", collapse = "|"))
  result <- str_squish(result)
